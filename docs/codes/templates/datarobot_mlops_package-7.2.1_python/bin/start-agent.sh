...
if [[ -z "$MICRONAUT_CONFIG_FILE" ]] ; then
    MICRONAUT_CONFIG_FILE="$TOP_DIR"/conf/mlops.agent.micronaut.yaml
fi
export MICRONAUT_CONFIG_FILES=$MICRONAUT_CONFIG_FILE

####### Micronaut static resources #######

if [[ -z "$MICRONAUT_JVM_DOCS_OPT" ]] ; then
    MICRONAUT_JVM_DOCS_OPT="-Dmicronaut.router.static-resources.docs.paths=file:$TOP_DIR/docs/html"
fi

if [[ -z "$MICRONAUT_JVM_UI_OPT" ]] ; then
    MICRONAUT_JVM_UI_OPT="-Dmicronaut.router.static-resources.default.paths=file:$TOP_DIR/ui/build/"
fi

##########################################

# Set the default AGENT_LOG_PATH
if [[ -z "$AGENT_LOG_DIR" ]]; then
    AGENT_LOG_PATH="$TOP_DIR"/logs/mlops.agent.log
    AGENT_OUT_PATH="$TOP_DIR"/logs/mlops.agent.out
else
    AGENT_LOG_PATH="$AGENT_LOG_DIR"/mlops.agent.log
    AGENT_OUT_PATH="$AGENT_LOG_DIR"/mlops.agent.out
fi
    ...
    if [[ -f "$AGENT_PID_FILE" && -r "$AGENT_PID_FILE" && -w "$AGENT_PID_FILE" ]]; then
        ...
        if [[ -n "$agent_pid" ]]; then
            if [[ $(ps -p "$agent_pid" -o comm=) =~ "java" ]]; then
                ...
                echo "DataRobot MLOps Agent was previously running with process id $agent_pid."
                echo "It may have been killed or exited with an error."
                echo "Check MLOps Agent log file for errors (default log file: $AGENT_LOG_PATH)."
                echo "Before restarting, remove the MLOps Agent pid file: $AGENT_PID_FILE."
                exit 1
            fi
        fi
    else
...
nohup ${CMD_JAVA} ${AGENT_JVM_OPT} "${MICRONAUT_JVM_DOCS_OPT}" "${MICRONAUT_JVM_UI_OPT}" \
